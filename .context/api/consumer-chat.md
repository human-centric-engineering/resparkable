...
**`scope` carrier.** An optional opaque `Record<string, string>` threaded verbatim into every capability dispatch this turn as [`CapabilityContext.scope`](../orchestration/capabilities.md#dispatch-scope-carrier-capabilitycontextscope). Core names no keys and no built-in reads it — **inert in vanilla Resparkable**. A fork uses it to surface-scope a conversation (e.g. `{ module, role }`) without shadowing this route. Because the value arrives on an untrusted end-user request it is bounded here (unlike the admin/persisted carrier). **Security:** a fork that reads `scope` to make an _access_ decision must re-validate it against the user's real entitlements server-side — a consumer-supplied scope is a routing/context hint, never proof of authorization. The platform holds to that itself: this carrier is threaded with `hintScope()` rather than `platformScope()`, so it can never bind the arguments of a capability that declared [`scopedBy`](../orchestration/capabilities.md#the-scope-binding-scopedby-dispatch-steps-4b--7a) — an earlier cut of that feature armed on the presence of a scope map alone, which let an end user set `priority` on every `escalate_to_human` call (#586).
...
| `PDF_NOT_SUPPORTED` (SSE)   | —      | Resolved chat model lacks the `'documents'` capability                   |
