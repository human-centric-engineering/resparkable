...
| #   | Deliverable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Verifiable by                                                                                                                                                                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 45  | **The key migration, alone in its own reviewed branch. DONE 2026-08-29.** `userId` → `spaceId` across `ResparkableSpace` and all **23** satellites (not 21: see below), `kind` + `ownerUserId` (indexed, **not** unique), `isDefault` + `name` + `slug` + `archivedAt` + the partial unique index (§24.1), **`createdByUserId` on every satellite (§23.5), taken here rather than in phase 48** so the tier's largest tables are migrated once, the schedule scope key **added beside** the old one rather than replacing it (§24.2), the hand-written FK moved, a `CHECK` making the ownership invariant a database rule, `SpaceScope` replacing `OwnerScope`, ESLint boundary and drift probes rewritten (B1 by definition, plus new B1b, B10, B11, B12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `db:drift-check` green; the whole existing test suite passes; **zero rows rewritten**, asserted by row count, content checksum, `pg_class.relfilenode` and `pg_stat_user_tables` tuple counters before/after |
| 46  | **DONE 2026-09-01.** `ResparkableGroup` + `ResparkableGroupMember` + **`ResparkableGroupInvite`** (a third table: a grant is live before acceptance and can carry its invite on one row, a group invitation must grant nothing until accepted, and folding it in needs a nullable `userId` that voids `@@unique([groupId, userId])`) + migration + probe B13 + roles + invite flow reusing §13's token shape + membership CRUD routes + the cross-subject export collector (in `access/subject-export.ts`, not core's manifest: every Resparkable table sits outside it under sunrise#533) + `transfer/policy.ts` dispositions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | last-admin rules; an invite grants nothing until accepted; export manifest test passes without deleting a row                                                                                                |
| 47  | **DONE 2026-09-03.** `?space=` on the URL as the one carrier (absence = personal; a repeated param resolves to nothing), switcher in the shell header, `requestSpaceScope()` replacing the mint in ~50 routes, `readResparkable()`'s **required** third argument, a tier-owned browser client that carries the workspace, per-space workspace tab state, the `/resparkable/groups` section with the full `ui.md` checklist, capture moved to `/capture` with an explicit target that defaults to personal and ignores `?space=` outright, sensitivity as a capture-time warning, and the agent layer following the space (`requireResparkableSpace()`, `loadResparkableContext` reading its `id`, `contextId` = the resolved space). Two of the six "paths" cannot write a thought at all here: `/transcribe` and `/transcribe/image` return text into the capture box                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | capture defaults to personal on every one of the six paths, asserted per path; `/capture` ignores `?space=` even when the caller IS a member of the space it names                                           |
| 48  | **DONE 2026-09-18.** Erasure settles a member's groups inside `eraseUser`'s transaction (`planErasureSuccession` + `settleGroupsAfterErasure`: promote the longest-standing joined member when the last admin goes, delete the space when the last member goes), group invitations scrubbed by address like grants, `DELETE /groups/[id]` requires `{ confirmName }` checked by the service and emails every other member, **`createdByUserId` written for the first time** (`authoredBy(scope)` at every scoped create; phase 45 added the column and nothing wrote it), the Art. 15 `groupContributions` section (rows the subject wrote, by group), the 13b route sweep and a real-database smoke                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | tests 13a–13e                                                                                                                                                                                                |
| 49  | **DONE 2026-09-24.** `granteeSpaceId` on `ResparkableGrant` (cascades from the grantee's space, `granteeEmail` now nullable, CHECK `one_grantee` + probe B14: a person or a group, never both), `spaceId` kept as the grantor column rather than renamed, `ResparkableViewer.group` making `/shared` per-space and **exclusive** (a group workspace holds only the group's grants, the personal one only the person's), membership re-resolved per request through `requestSpaceScope`, a group `viewer` never comments, `POST /grants` taking `granteeSpaceId` for a group the caller is a joined member of (never its own), `GET /grants/groups` with member counts for the dialog's Groups tab, no invite for a group grant. Fixed on the way: a group's grant to a person never listed (now "Shared by <group>"), and `/shared/search` ignored the workspace. Group-owned items still do not resolve as `owner` (phase 58); a group still cannot email an invite                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | tests 13f–13g (`smoke-group-sharing`); §13's tests 3–4 still pass unchanged                                                                                                                                  |
| 50  | **DONE 2026-09-25.** `assertCanSpend` (viewer refused, group balance only, never the member's own, `dailyCreditCap` over a rolling 24 hours, all before the provider call), every ledger row attributed (`createdByUserId`), runs carrying the space they are about, group spaces owed group jobs only, `backgroundSpaceScope`. The digest: `group_digest` job (the group's Monday 09:00, `gatesFirstRun` so even a first run over a quiet week is not billed), `resparkable-group-digest` workflow, `resparkable-digester` agent, `resparkable_get_group_digest_inputs` (a gather with no authorship in it), and a write-time guard in `writeReview` that refuses a `group_digest` naming a member or comparing people, whoever writes it; the newest digest on the group page. The budget: `fundingMode` (`self_funded` or `member_contributions`, the 2026-09-25 amendment), top-up as a one-way `transfer_out`/`transfer_in` pair from the giver's own balance that refuses rather than overdraws, per-member daily caps set by an admin, the two admin-only email alerts (low-balance on the crossing; large run as a share of what was left, at most once in 24 hours and not once the balance is already below the low mark), per-person spend and contributions over 30 days for admins only, and a member's group ledger rows in their Art. 15 export. **Known gap, accepted 2026-09-25:** a group run a member started is stored against that member (`AiWorkflowExecution.userId`, which cascades), so if they erase their account in the minute before the billing pass reaches it, the run goes with them and the group is never charged for it. Small and short-lived; revisit if group runs grow expensive. Fixed on the way: a group's inbox token reached `resparkable_capture_for_token`, which treated the group key as a user; now refused as `group_inbox_unsupported` | test 13h (the digest is non-comparative) and test 13k                                                                                                                                                        |
| 57  | **DONE 2026-09-25.** 23.11 joining: hashed join links with §13's public-link discipline (`maxUses`, expiry, revocation, role fixed at mint and never `admin`), open-vs-request approval as a `joinedAt: null` membership row, `maxMembers`, the two new rate-limit tiers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | test 13i; a pending member resolves to no scope at all                                                                                                                                                       |
...
| 60  | **DONE 2026-09-20.** Self-service MCP keys: the Connect card on `/resparkable/settings`, four `spaces/:spaceId/mcp-keys` routes, one live key per person per workspace with every field forced or derived, a dispatch guard that **refuses** a scope carrier this tier cannot read instead of falling through to the default workspace, per-client setup snippets, and erasure deleting a person's own scoped keys. Org binding (D3) waits on Sunrise 0.13, which is untagged. Designed in [`connect-assistant-plan.md`](./connect-assistant-plan.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | tests in `mcp/key-scope*.test.ts`, `mcp/keys.test.ts`, `mcp-keys.routes.test.ts`; a real scheduled-run context passes the guard unchanged                                                                    |
| 62  | 23.12's second amendment: who pays, and the group's books (23.14 q7 to q10, answered 2026-09-25). On `ResparkableGroup`: a seat price per role; `operationalCostsPaidBy` (shared balance, founder, a chosen admin, or admins split); `activityCostsPaidBy` (one pool, equal split, or each member's group allowance, set by the admin and never a personal balance); a shortfall policy (debt to the group, or temporary loss of access). `assertCanSpend` and `recordAgentSpend` resolve the account(s) to charge from those settings before the run, and never switch accounts on an empty balance; an equal split is one transaction across every member. A group accounting view for **every member** (collective spend, per-person spend on the group, seats, top-ups, by week and category), replacing phase 50's admin-only table; every setting change in 23.13's audit log. A monthly cycle for seats and budgets, with unused budget rolling over and a pre-agreed, capped automatic top-up from the member's personal balance when a month's budget runs out; allowance source (carved from the pool, or filled by the member) as a setting; unused balance credited back on leaving, removal, erasure or group deletion, in the same transaction. Design doc first: what a member's share of a pool is on refund, and what happens to a debt on leaving                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | test 13n; 13h and 13m still pass unchanged                                                                                                                                                                   |
...
13. **Groups (§23).** The migration first, because everything else is downstream of it.
    13a. **The key migration is lossless.** Row counts, a content checksum, `pg_class.relfilenode` and the `pg_stat_user_tables` tuple counters are compared per satellite table before and after phase 45 by `npm run framework:resparkable:key-checksum`. The invariant is asymmetric and stated that way because the symmetric version is unassertable: **zero writes to the 23 satellites, exactly one write per existing row on `ResparkableSpace`**, which gains six columns and a backfill. The relfilenode is the measure that answers the question; a checksum alone is invariant under a full table rewrite, which is what a rewrite is. `db:drift-check` is green, probe B1 finds the hand-written FK **on `ownerUserId` by definition rather than by name**, and B1b finds the old one gone.

    _Reworded 2026-08-29._ This row used to require that "every pre-existing test passes with no edit beyond the `OwnerScope` → `SpaceScope` rename", and that is not achievable: this tier's tests assert the object handed to a mocked Prisma, so a field rename is visible to them by construction, and a version that left them passing would mean they were not asserting the column. Satisfying it literally would mean keeping a permanent `@map`, which is the wrong trade. What replaces it is stronger and machine-checkable: **no test's intent changes, and every edit is a rename inside an assertion**. The removed and added lines are identical after token substitution, per-file `expect`/`it`/`describe` counts are unchanged, no `.skip`/`.only` appears, and **no string literal changes**, which holds because the migration changes names and never values.
    ...
    13n. **The books show money, never output.** With activity costs set to each member's allowance, a member's chat turn debits their group allowance, never the group's pool and never their personal balance, and an empty allowance is refused. With costs shared and the pool empty, the turn is refused rather than charged to a member. With an equal split, a member who cannot cover their share follows the group's shortfall policy (their allowance goes negative, or they lose access), and the split is all-or-nothing in one transaction. Nothing ever switches accounts on an empty balance. A seat is charged at its role's price. Every member, not only admins, can read the group's accounting, and it shows credits by person, category and week, contains no count of items written, tasks finished or messages sent, and no ordering by any of them. An automatic top-up happens only up to the amount the member agreed, and never without that agreement. Unused budget rolls into the next month. A member who leaves, and every member of a deleted group, is credited back their unused balance in the same transaction. The digest bills as an operational cost to whoever the settings name.
...
> **Amended 2026-09-25: funding is the admin's choice.** A group admin sets how
> the group is funded: **self-funded** (admins top the balance up) or **member
> contributions** (any member may move credits from their own balance into the
> group's). Both are explicit, one-way transfers that a person makes on purpose,
> and neither changes the rule below: at spend time exactly one account is read,
> the group's, and an empty group balance is a refusal. What the amendment
> removes is only "not a pool assembled from members' personal balances"; a
> contribution is a deliberate gift to the group, never a fallback taken without
> asking. Who contributed what is admin-visible like spend, and never shown to
> members (23.8). Ships in phase 50's second half.
...
> **Amended 2026-09-25 (second): who pays, and the group's books.** Phase 50
> shipped one pool: every cost comes out of the group's balance, and members see
> the balance but not who spent it. That is one setting among several a group
> should be able to choose, and the books should be open to the group. Phase 62.
>
> **Two kinds of cost.** _Operational_ costs are runs nobody in particular
> started: today only the weekly digest bills, and anything scheduled per group
> later joins it. _Activity_ costs are runs a member started: chat, ideate,
> summarise, a workflow they triggered. Every ledger row already records who
> started it (`createdByUserId`, null for operational), so the split needs a
> setting, not a new record.
>
> **Two settings, each the admin's choice.** Operational costs are paid by the
> group's shared balance or by the group's owner. Activity costs are either
> shared (the group's balance, as today) or paid by the member who ran them, from
> their own budget. With activity costs on "each member pays their own", a
> member's chat turn in the group debits that member.
>
> **What survives from the rule above.** The rule against falling back to a
> member's own balance stays, in this form: **at spend time exactly one account
> is read, it is decided by the group's settings before the run, and an empty
> account is a refusal, never a switch to a different one.** A member who pays
> their own activity costs knows that in advance, because the setting is shown to
> every member and changing it is a logged admin action (23.13's audit log). What
> the rule forbade was a silent redirect when the pool ran dry, and that is still
> forbidden: a shared-costs group with an empty balance refuses, and does not
> start charging members.
>
> **Open books.** This reverses "members see nothing about who spent it". Every
> member can see the group's accounting: collective spend (operational, and
> shared activity) and each person's spend on the group, plus top-ups. The line
> 23.8 draws moves rather than disappears: **the books show money, never output.**
> Credits by person, by category and by week; never counts of items written,
> tasks finished or messages sent, and never a ranking by any of them. The digest
> (13h) and the feed (13m) are unchanged. They still rank nobody. An account of
> who spent what is a fair question in a group paying a shared bill; a table of
> who produced what is the surveillance 23.8 refuses.
>
> **Role-tiered seats.** Holding a seat costs credits at a price the admin sets
> per role (23.14 q7). A viewer still spends nothing on activity, because a
> viewer cannot start a billed run, but may pay for their seat.
>
> The settings this amendment adds, as answered on 2026-09-25 (23.14 q7 to q10):
> seat price per role; operational costs paid by the shared balance, the founder,
> a chosen admin, or the admins split between them; activity costs shared as one
> pool, split equally, or paid from each member's group allowance, which the
> admin sets and which never touches a personal balance; and a shortfall policy
> for a member who cannot pay their share (debt to the group, or temporary loss
> of access).
...
7. **Role-tiered credit costs. Answered 2026-09-25: a seat price by role.**
   Holding a seat in the group costs credits, at a price the admin sets per role
   (admin, member, viewer). It is an admission cost, not a rate on activity: the
   same chat turn costs the same whoever runs it. There is no "editor" role; the
   roles are admin, member and viewer.

   **Monthly, with pre-agreed top-ups and roll-over (answered 2026-09-25).** Seats
   and budgets run on a monthly cycle. Because real costs depend on activity, a
   member agrees **in advance** to an automatic top-up amount, taken from their
   personal balance when their monthly group budget is exceeded. This is not the
   silent fallback phase 50 forbids: it is a standing instruction the member set
   and can see and withdraw, capped at the amount they agreed, and each top-up is
   its own ledger row they can see. Unused budget from one month **rolls over** to
   the next.

   **Money goes back when a member leaves or the group closes.** A member who
   leaves (or is removed, or is erased) is credited back their unused balance in
   the group, and when a group is deleted every member is credited back theirs,
   inside the same transaction as the leave or delete. Still to settle in the
   design doc: what "their" unused balance is when money sat in a shared pool
   (for example, what they put in, less what was spent on their behalf and their
   share of shared costs), and what happens to an outstanding debt (23.14 q8) on
   leaving.

8. **Splitting shared costs equally. Answered 2026-09-25: configurable, including
   what happens when someone cannot pay.** "Shared" can mean one pool, or each
   shared cost divided into equal charges, one per member. When a member cannot
   cover their charge, the group's settings decide what happens, with at least
   two options: the member **goes into debt with the group** (their account runs
   negative and is repaid by their next top-up), or they are **temporarily denied
   access** until it is settled. A split across thirty members is one ledger
   write per member, done in one transaction, so a part-paid split is never
   visible.
9. **A member's own budget. Answered 2026-09-25: an allowance the admin sets,
   inside the group.** A member who pays their own activity costs pays from a
   group allowance, never from their personal balance. The admin sets the
   allowance; nothing reaches into the member's own brain's credits. This keeps
   the phase-50 rule that a group's bills never touch a personal balance
   uninvited. **Where an allowance comes from is also the admin's choice
   (answered 2026-09-25):** carved out of the group's balance (a limit on how
   much of the pool that member may use) or filled by the member's own top-ups
   (money that is theirs to spend in the group). Both are offered.
10. **Who pays operational costs. Answered 2026-09-25: all three, as a setting.**
    The group's configuration chooses the founder, a chosen admin, or the admins
    split between them. If the chosen payer leaves or is erased, the setting falls
    back to the group's shared balance and the admins are told, rather than
    running costs failing silently.
...
| MCP tools                               | Key owner implies the space                            | **DONE, phase 60.** The key names one workspace, re-resolved through membership on every call, and a carrier this tier cannot read is refused rather than falling through to the default |
