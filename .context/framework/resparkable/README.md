...
| File                                                     | What it is                                                                                                                                                                                                                                                                                   |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`plan.md`](./plan.md)                                   | The full implementation plan: data model, migrations, API, agents, workflows, prioritisation, lifecycle, boards, sharing, Obsidian sync, **Cross-Pollination (§18)**, **Instruct mode (§19)**, **billing (§20)**, **Situations (§21)**, **Present mode (§22)**, phasing, verification, risks |
| [`phase-45-plan.md`](./phase-45-plan.md)                 | Phase 45 in full: why the rename costs nothing, the transitional `@map` that separates the irreversible commit from the churny one, why object names are harvested from Prisma rather than derived, and the four places §23 was written against an older schema                              |
| [`release-9-plan.md`](./release-9-plan.md)               | Release 9 (Groups, `plan.md` §23) sequenced for build: nine phases across four branches, the `userId` to `spaceId` key migration in commit-by-commit detail, and five places where the code has moved on since §23 was written                                                               |
| [`phase-46-plan.md`](./phase-46-plan.md)                 | Phases 46 to 48 (Branch 2 of Release 9): why a group invite needs its own table when a grant does not, which cascade reaches the three new tables, the one file membership is allowed to be read in, and the privacy departure from the release plan                                         |
| [`phase-57-plan.md`](./phase-57-plan.md)                 | Phase 57 (Branch 4 of Release 9): join links as bearer credentials, why the link table names no person, the row lock behind the member cap, and why "tells the admin" is not an email                                                                                                        |
...
| [`sharing.md`](./sharing.md)                             | The access layer (Release 2): the two-layer D5 boundary, `ResparkableGrant` / `ResparkableShareLink`, the owner short-circuit, what each basis redacts, the cascade and the dynamic-filter trap, four deliberate deviations from the plan                                                    |
...
| [`mcp.md`](./mcp.md)                                     | What the brain exposes over MCP and why that list _is_ the access control: the `scopedAgentId` gotcha, the eight tools, the three prompts, the two resources and the scope a card key lacks for them, the operator setup                                                                     |
...
| [`workspace-shell-smoke.md`](./workspace-shell-smoke.md) | The three-pane shell's manual smoke pass: 21 items, each naming what would be **wrong** rather than only what to do. Still unrun — both bugs found since the cutover were caught by a person looking at a screenshot, not by a test                                                          |
...
**Release 9, phase 57 landed, 2026-09-25** ([`phase-57-plan.md`](./phase-57-plan.md), Branch 4 of [`release-9-plan.md`](./release-9-plan.md)). The point at which somebody can join a group without an admin typing their address. An admin mints a join link at `member` or `viewer` (never `admin`, refused at the schema and again in the service), either `open` or needing approval, with an expiry and an optional use limit. **The one thing to know before extending this: a join link names nobody**, so unlike an invitation a forwarded link is the whole credential, and it takes §13's public-link discipline rather than the invite's. A `request` link writes a membership row with `joinedAt: null` that resolves to no scope at all until an admin lets them in. `maxMembers` is enforced on redemption and approval under a `FOR NO KEY UPDATE` lock on the group row, so racing joins cannot overrun it; a refusal is recorded for the admins to see rather than emailed. Test 13i is the unit suite plus a real-database smoke, `npm run framework:resparkable:smoke-group-join`, which races six joins at a cap of two. Fixed on the way: removing a pending request from a group whose only joined member was the admin deleted the group.

**Release 9, phase 48 landed, 2026-09-18** ([`phase-46-plan.md`](./phase-46-plan.md), Branch 2 of [`release-9-plan.md`](./release-9-plan.md)). The point at which a person can leave the product without taking a group with them, or leaving it stranded. Erasing a member settles their groups inside the erasure transaction: the last admin's role passes to the longest-standing member, and a group with nobody left is deleted with its space. Deleting a group on purpose takes its name typed back, checked by the service rather than only the dialog, and emails every other member. A subject's export now includes, per group, the rows they wrote there and nobody else's. **The one thing to know before extending this: a row now records who wrote it.** Phase 45 added `createdByUserId` and nothing wrote it until this phase; `authoredBy(scope)` in `repo/space-scope.ts` is how a create gets it, `isolation.test.ts` fails a scoped create that does not, and the export's group section reads it. Test 13b is a sweep over all 74 workspace routes plus a real-database smoke, `npm run framework:resparkable:smoke-group-erasure`. Still open: Sunrise ask #44 now covers succession too, and ask #47 is why the export's narrowing is stated in a description rather than a `scopeNote`.

**Release 9, phase 47 landed, 2026-09-03** ([`phase-46-plan.md`](./phase-46-plan.md), Branch 2 of [`release-9-plan.md`](./release-9-plan.md)). The point at which a person can open a group's workspace. Which workspace a request is for travels as **one search param**, `?space=`, with absence meaning the personal space, because §24.2 required it be in the URL and a search param leaves every existing bookmark and route resolving unchanged. The switcher is in the shell header and a switch is `router.push` to the same page with a different `?space=`, which is what makes a workspace a link and the back button work between workspaces without either being built. Around fifty routes stopped minting their own scope in favour of `requestSpaceScope()`, so the greppable trust boundary got _shorter_. **The one thing to know before extending this: a capture is aimed, everything else follows the room you are in.** `POST /capture` takes its target from the body, defaults to personal on every path, and ignores `?space=` outright even when the caller is a member of the space it names, because a thought landing in a shared brain through stickiness is a failure with nothing on screen to show it. Every other write follows the workspace you are looking at, like any other CRUD create. The agent layer follows the space too (§23.9), and re-resolves the space it is told through membership rather than trusting it. Group deletion, admin succession and the Art. 15 predicate are phase 48's.

**Release 9, phase 46 landed, 2026-09-01** ([`phase-46-plan.md`](./phase-46-plan.md), Branch 2 of [`release-9-plan.md`](./release-9-plan.md)). The point at which a group exists and can be joined. Three new tables (`ResparkableGroup`, `ResparkableGroupMember` and a `ResparkableGroupInvite` §23.3 did not ask for), an additive migration that alters no existing table, probe **B13**, a membership service that is the only place a group scope may be minted, seven API routes, and the invitation flow. Two things worth knowing before extending it. **A group invitation is inert until accepted**, unlike a share grant, which is live for its address the moment it is written: membership is never resolved by email anywhere, so the token is a real credential in a way a share invite's is not, and the accept page therefore asks before it acts. And **the group row cascades from its space and not the reverse**, which makes deleting a group a single `DELETE` of the space row that the D1 cascade follows through all 23 satellites. No group UI beyond the accept page yet: the space switcher and the six capture paths are phase 47's, erasure succession and the Art. 15 predicate phase 48's.

**Release 9, phase 45 landed, 2026-08-29** ([`phase-45-plan.md`](./phase-45-plan.md), sequenced in [`release-9-plan.md`](./release-9-plan.md)). The structural half of Groups, and nothing user-visible: the tier's owner key is now `spaceId` on `ResparkableSpace` and all 23 satellites, the GDPR cascade moved to a new nullable `ownerUserId`, and `OwnerScope` became `SpaceScope { spaceId, actorUserId, role }` so the partition key and the acting person are no longer the same field. The migration rewrites no rows (a catalog rename over 23 tables, proven by `npm run framework:resparkable:key-checksum` comparing relfilenodes and tuple counters, not just checksums). It also carries Release 10's workspace columns and §23.5's `createdByUserId`, because the migration that would add them is the migration over the tier's largest tables and it happens once. Four new drift probes: B1b, B10, B11 and B12. Nothing creates a group space until phase 46.
...
**The workspace shell rework landed 2026-08-19 through 2026-08-20** (its own Phases 0–9 — a distinct sequence from Release 1's Phase 9 above, which is about PWA/voice/image capture channels and is unaffected by this; the two just happen to share a phase count). Replaced the left `ResparkableNav` rail, the `/resparkable/chat` destination page and the right-edge `ResparkableSidekick` capture drawer with a persistent three-pane shell: an always-visible **Sparkey** pane (chat + capture + instruct, left), a fluid tabbed/splittable **Workspace** pane (centre), and an **Activity** pane (a live discovery feed, right), tiled with `react-resizable-panels`. Both side panes collapse to a thin `PaneRail` rather than vanishing; a tab can be dragged out of the tree into its own floating, resizable window (`FloatingTabWindow`/`FloatingPanelsLayer`) and redocked; the old nav rail's destinations move into `Launcher`, a per-pane grid of tiles sized by container query rather than viewport; and a full-viewport **Present** mode (deck-from-Graph-selection, or on-the-fly dictated slides) opens from the header. `ResparkableNav`/`ResparkableSidekick` were orphaned by the cutover and deleted in the 2026-08-22 consolidation pass, along with the retired Ask Sparkey nav/help entry (Sparkey is a permanent pane, not a destination). `QuickCapture` stays: it is still what `/resparkable/capture`, the Android share-target landing page, renders. See [`ui.md`](./ui.md) §§10–14 for the shipped shape and `plan.md`'s top banner + §9 for how this relates to the original design-stage scoping note.

**A consolidation pass followed on 2026-08-22**, closing the three shell follow-ups that were reachable in the live product rather than only in the plan. Two shared-state bugs, both invisible as failures because everything worked, just in the wrong pane: Plan/Projects/Search read their filters from the shared browser URL, so two panes of the same kind moved together, and _every_ mutating control in the tier finished with `router.refresh()` — thirty-five call sites across twenty-two files — which refetches the whole route segment all three panes sit under while doing nothing at all for a launcher-opened tab's own data. The first is fixed by giving filters a home in `TabParams` plus one optional, navigating-by-default callback per view; the second at the seam, by `useResparkableRefresh()`. Alongside them: `TabState.title` is finally written, so detail tabs read "Q3 Roadmap" rather than "Project", and the inbox count the deleted nav rail used to carry is back on the Launcher's Inbox tile (the connection count is not, and deliberately: the Activity pane replaces it). See [`ui.md`](./ui.md) §15 for the two rules this leaves behind, and [`workspace-shell-smoke.md`](./workspace-shell-smoke.md) for the manual pass that still has not been run.
...
| Drift probes                            | `lib/app/db-drift.ts` → `registerResparkableDriftProbes()` (seven, B1–B7; B3 and B7 are **forbidden-object** probes that fail if the index comes back)                                                                                                                                                                                                                         |
...
| Job queue                               | `framework_resparkable_job`: seven kinds per personal brain (`triage`, `briefing`, `weekly_review`, `horizon_check`, `sweep`, `retention`, `reindex`), four per group (`group_digest` and the three maintenance passes). `queue/kinds.ts` holds the cadences                                                                                                                   |
...
| Schema                                  | 26 models in `prisma/schema/framework-resparkable.prisma` (`ResparkableSpace` plus 23 satellites keyed on `spaceId`, plus two operator singletons)                                                                                                                                                                                                                             |
...
| Repo layer                              | `lib/framework/resparkable/repo/*` — `SpaceScope` (`space-scope.ts`, renamed from `OwnerScope` in phase 45), 26 modules (`retention` and `stale` in phase 8)                                                                                                                                                                                                                   |
...
| MCP exposure                            | `prisma/seeds/framework-resparkable/006-mcp.ts` from `lib/framework/resparkable/mcp/exposure.ts` — eight tools and three prompts, **no code** (phase 7b), plus two resources whose handlers are `lib/framework/resparkable/mcp/resources.ts`: core's resource path runs no capability guards, so that module repeats the owner, carrier and membership checks itself           |
...
Everything in the brain is a **space query**. `SpaceScope` is a branded type minted only from a verified session id (or, since Release 2, from a resolved grant), every repo function takes one, and three boundaries keep it honest. Since phase 45 it carries `actorUserId` and `role` alongside the partition key, and **the actor is never a filter**: `repo/isolation.test.ts` asserts that by enumeration over every exported function and every raw statement.
...
1. **The owner is resolved before a subclass runs.** `ResparkableCapability` mints
   the `SpaceScope` from `CapabilityContext.userId` and hands it to `run()`, so a
