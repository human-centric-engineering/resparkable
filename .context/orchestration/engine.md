...
| `tool_call`     | `unknown_capability`                  | No capability class is registered under that slug. Since #537 the executor calls `registerBuiltInCapabilities()` first, so this no longer means "the process was cold" — it means the slug really is unregistered (a typo, or a fork capability whose `registerAppCapability` never ran)                                                                                                                                                                                                                                                                                                                         |
| `tool_call`     | `capability_inactive`                 | Capability exists but is not active                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `tool_call`     | `capability_disabled_for_agent`       | **Unreachable from `tool_call` since #528.** A workflow dispatches under a `workflow:${workflowId}` label, which the dispatcher now resolves to the base capability's defaults in both binding modes — so there is no row to disable and no strict-mode denial. Previously `CAPABILITY_BINDING_MODE=strict` raised it on every step, with no row an operator could create to fix it (the `AiAgentCapability` FK rejects the label). Kept in the executor's permanent-error list defensively                                                                                                                      |
| `tool_call`     | `invalid_args`                        | Arguments failed Zod validation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `tool_call`     | `requires_approval`                   | Capability requires admin approval                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `agent_call`    | `agent_call_budget_exceeded_per_turn` | Accumulated step cost crossed the per-turn cap (`agent.maxCostPerTurnUsd` or org default `defaultMaxCostPerTurnUsd`). Mirrors the chat streaming handler's guard: re-running with the same prompt will hit the same cap, so the workflow's error strategy (skip / continue / failure-branch) decides routing. The ExecutorError carries partial `tokensUsed` / `costUsd` so the trace + cost rollup reflect what was actually spent.                                                                                                                                                                             |
| any LLM step    | `provider_not_permitted`              | The step has no `modelOverride`, so Sunrise resolved the `chat` task default — and the provider that model names is barred by the rule registered via `registerProviderEligibility()` (or that rule threw, which denies). Deterministic within the run, so retrying would spend the whole budget on the same answer. Raised before the provider is constructed, so nothing was sent. Affects `llm_call`, `route`, `evaluate`, `reflect`, `guard`, `supervisor` and `orchestrator`; a step that names its own model is deliberately not filtered. See [`llm-providers.md`](./llm-providers.md#per-path-coverage). |
