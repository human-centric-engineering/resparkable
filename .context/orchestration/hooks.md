...
tool set the model actually had, so a reviewer can see what it invented the name
from. The repeated-failure circuit breaker (`tool_unavailable`) deliberately
does **not** emit this event — that one is the breaker doing its job, and it is
already in the logs. Both refusals do emit a `warning` SSE frame carrying the
same code.
