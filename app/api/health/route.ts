 ...
 * edit, not two.
 *
 * **The Resparkable platform version is deliberately not here.** This endpoint is
 * unauthenticated — load balancers and container orchestrators probe it — so
 * anything it returns is returned to anyone. `RESPARKABLE_VERSION` names the exact
 * upstream release a deployment runs, and therefore the exact set of published
 * issues to try against it, across every Resparkable-derived deployment rather than
