...
# Overridable for a fork that renamed its copy of `lib/sunrise-version.ts` /
# `SUNRISE_VERSION` as part of its own rebrand (Resparkable did, at the
# Obsiddy→Resparkable rename — see VERSIONING.md). Unset falls back to
# Sunrise's own names, so this is a no-op for Sunrise itself and for any fork
# that left the file alone.
VERSION_FILE="${VERSION_FILE:-lib/sunrise-version.ts}"
VERSION_CONST="${VERSION_CONST:-SUNRISE_VERSION}"
...
VERSION=$(git show "$REF:$VERSION_FILE" 2>/dev/null |
  sed -n "s/.*$VERSION_CONST *= *['\"]\([^'\"]*\)['\"].*/\1/p" | head -1)

if [ -z "$VERSION" ]; then
  skip "no $VERSION_CONST found at $REF"
fi
TAG="v$VERSION"
...
TAG_VERSION=$(git show "$PRIVATE_REF:$VERSION_FILE" 2>/dev/null |
  sed -n "s/.*$VERSION_CONST *= *['\"]\([^'\"]*\)['\"].*/\1/p" | head -1)
