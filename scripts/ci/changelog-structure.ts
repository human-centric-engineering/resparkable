...
export function checkChangelogStructure(
  ...
  if (!latest) {
    violations.push({
      line: 0,
      message: `No release headings found, but \`RESPARKABLE_VERSION\` is ${options.sunriseVersion}. Expected \`## [${options.sunriseVersion}] — YYYY-MM-DD\`.`,
    });
  } else if (latest.version !== options.sunriseVersion) {
    violations.push({
      line: latest.line,
      message: `Topmost release is ${latest.version} but \`RESPARKABLE_VERSION\` in lib/resparkable-version.ts is ${options.sunriseVersion}. Cutting a release bumps both in the same PR (CONTRIBUTING steps 2 and 4).`,
    });
  }
