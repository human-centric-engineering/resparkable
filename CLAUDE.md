...
| Design Language          | `.context/ui/design-language.md`                          | Palette, the three fonts, radii, `.term-*` / `.lattice-*` classes, the 11px floor, the cascade rule                                                                                                               |
...
| Local dev hostname       | `.context/framework/resparkable/dev-proxy.md`             | Dev runs on `https://resparkable.test`, not `localhost:3016`: shared HCE proxy registry, `apply.sh`, the `herd unproxy` gotcha, no Google OAuth on `.test`                                                        |
| Scale ambition           | `.context/framework/resparkable/scale.md`                 | The 100k–500k-user target: where each foundation breaks, decisions S1–S8, why the data model scales and scheduled compute does not                                                                                |
| Job queue design         | `.context/framework/resparkable/phase-56-plan.md`         | S6 in full: one `ResparkableJob` table, `SKIP LOCKED` claim, `dueAt` not cron, worker-as-a-loop, why Resparkable owns its own dispatcher                                                                          |
| Sharing (access layer)   | `.context/framework/resparkable/sharing.md`               | D5's two layers (`repo/**` owner queries vs `access/**` shared queries), grants, public links, invites, comments and erasure; the owner short-circuit, per-basis redaction, the cascade and the filter-board trap |
| Design Principles        | `.context/framework/resparkable/design-principles.md`     | Reflection over optimisation: no hour quotas, no "neglected" framing; why `areaBalance`/`weeklyCapacityMinutes` were removed rather than hidden                                                                   |
| Brand Assets             | `.context/framework/resparkable/brand-assets.md`          | The logo: faceted infinity loop + spark; which of the 9 SVGs to use, why the spark inverts between grounds, the `/resparkable/brand` page, how to regenerate                                                      |
...
**`https://resparkable.test` returns 502 / cert error:**

- 502 = nothing on port 3016. Start `npm run dev`, or `PORT` in `.env.development` has drifted from `dev-proxy/apps.json`
- Cert error = no proxy for that host. `cd ~/code/dev-proxy && ./apply.sh`
- Full detail: [`.context/framework/resparkable/dev-proxy.md`](./.context/framework/resparkable/dev-proxy.md)
