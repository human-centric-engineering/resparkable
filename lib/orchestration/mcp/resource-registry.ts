 ...
 * functions only.
 *
 * Resparkable's own resources use the `resparkable://` scheme; a fork registers its
...
/** The scheme Resparkable's own resource URIs use, without the `://`. */
const CORE_URI_SCHEME = 'resparkable';
...
export interface AppMcpResourceRegistration {
   ...
   * `hub://projects/{id}/plan`. Pass `'resparkable'` to extend the platform's own
   * scheme rather than introducing one.
   *
   * Required rather than defaulted: a fork resource that silently inherited
   * `resparkable://` would advertise the platform's identity to every MCP client
 ...
 * would let a fork file `resparkable://projects/x/plan` under its own
 ...
 *
 * **A built-in type cannot be overridden.** Resparkable seeds rows for its own
 ...
 * change what `resparkable://agents` returns to an external MCP client. That is a
...
/**
 * Whether a resource URI uses an allowed scheme — `resparkable://`, or one a fork
 * contributed via `registerMcpResourceHandler`.
 *
 * **Case-sensitive, deliberately.** An earlier version matched the scheme with
 * `/i` and lowercased it, which accepted `RESPARKABLE://agents` — a row that stores
...
/**
 * The URI scheme a resource of `resourceType` must use — `resparkable` for a
 ...
 * under `hub`, `{ uri: 'resparkable://projects/x/plan', resourceType: 'project_plan' }`
