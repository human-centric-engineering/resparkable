 ...
 *
 *   • `repo/**` — space queries. Every function takes a `SpaceScope` and
 ...
 * scope by.
 *
 * `mcp/keys.ts` is exempt for the same kind of reason, and it is worth being
 * precise about which kind. `McpApiKey` is a core-owned credential table with
 * no `spaceId` column, so no `SpaceScope` can filter it and `spaceWhere()` has
 * nothing to spread. Putting it in `repo/**` would mean writing the first repo
 * function whose `where` is keyed on the **actor**, which §23.2 forbids and
 * `isolation.test.ts` asserts against: an actor in a `where` in that layer is a
 * per-row ACL on forty list endpoints. A credential list is the one thing an
 * actor legitimately filters, and it does not belong in the layer built on the
 * opposite premise. The exemption is held to exactly that by
 * `tests/unit/lib/framework/resparkable/mcp/keys-boundary.test.ts`, which fails
 * if the file ever reaches a table other than `mcpApiKey`.
 */
const prismaBan = {
  group: ['@/lib/db/client', '@prisma/client'],
  allowTypeImports: true,
  message:
    'Only lib/framework/resparkable/repo/** (space queries) and lib/framework/resparkable/access/** ' +
    '(shared queries) may reach the database. Add a repo function that takes a SpaceScope — a direct ' +
...
export default [
  // ── Framework-tier import boundary ───────────────────────────────────────
  {
    ...
    ignores: [
      ...
      'lib/framework/resparkable/mcp/keys.ts',
    ],
    rules: {
  ...
  // ── D5: space queries and shared queries are structurally separate ───────
  // `lib/framework/resparkable/repo/*` takes a `SpaceScope` and must not be able
