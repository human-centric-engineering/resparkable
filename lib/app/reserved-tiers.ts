 ...
 * ```
 *
 * FORK (Resparkable): Resparkable is the framework-layer tier, so it fills the
 * two `/framework` entries below and deliberately leaves the leaf-fork tiers
 * (`components/app`, `.context/app`) and `components/framework` empty — those
 * stay guarded, because a host project installing Resparkable is entitled to
 * them. Its own components live in `components/resparkable/`, outside every
 * reserved tier, for the same reason.
 *
