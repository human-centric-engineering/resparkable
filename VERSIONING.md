...
fork, and container health checks and deploy scripts read it. `RESPARKABLE_VERSION`
is **not**: it is served from `GET /api/v1/admin/stats` as
`system.resparkableVersion`, behind `withAdminAuth`, and rendered on
`/admin/overview`.
...
across every Resparkable-derived deployment rather than just one — while the
