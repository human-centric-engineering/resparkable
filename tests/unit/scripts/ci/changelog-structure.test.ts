...
describe('checkChangelogStructure', () => {
  ...
  it('accepts the real CHANGELOG.md', async () => {
    ...
    const { RESPARKABLE_VERSION } = await import('@/lib/resparkable-version');
    const source = readFileSync('CHANGELOG.md', 'utf8');

    expect(check(source, RESPARKABLE_VERSION)).toEqual([]);
  });
  ...
  describe('RESPARKABLE_VERSION agreement', () => {
    ...
    it('reports a file with no releases at all', () => {
      expect(check('## [Unreleased]\n', '0.2.0')).toEqual([
        expect.stringContaining('No release headings found, but `RESPARKABLE_VERSION` is 0.2.0'),
      ]);
    });
  });
