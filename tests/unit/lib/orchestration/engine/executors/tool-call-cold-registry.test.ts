...
async function coldProcess() {
  // The upstream key is `sunriseCapabilityDispatcher`; this fork's
  // `dispatcher.ts` renamed the globalThis slot to `resparkableCapabilityDispatcher`
  // (predates this file). Deleting the wrong key is a silent no-op — the real
  // singleton survives across `coldProcess()` calls and the second test's own
  // precondition (`has(...)` false) fails, proving the leak instead of masking it.
  delete (globalThis as { resparkableCapabilityDispatcher?: unknown })
    .resparkableCapabilityDispatcher;
  vi.resetModules();
