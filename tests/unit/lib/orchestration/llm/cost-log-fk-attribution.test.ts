...
const ALLOWED_CALL_SITES: readonly string[] = [
  ...
  // ── FORK (Resparkable) ──────────────────────────────────────────────────
  // Three tier call sites, each answering the question this list exists for:
  // can `agentId` ever hold something that is not an `AiAgent` row?
  //
  // Voice and photo capture: `agent` is `gating.value.agent`, the row
  // `resolveCaptureRouteGating()` loaded before the route did any work — the
  // route cannot reach `logCost` without it. Neither surface has a
  // conversation: the transcript becomes the user's own note rather than a turn
  // addressed to the agent, which is why the companion is resolved for
  // attribution and model support and for nothing else.
  'app/api/v1/resparkable/transcribe/route.ts | agentId=agent.id | conversationId=— | workflowExecutionId=— | userId=—',
  'app/api/v1/resparkable/transcribe/image/route.ts | agentId=agent.id | conversationId=— | workflowExecutionId=— | userId=—',
  // Ideation: `findAgentBinding()` returns the bound `AiAgent` row or null, so
  // the column is written only when there is a row to point at — the spread is
  // the guard, not decoration. Attributed to no conversation and no execution:
  // ideation runs from a service, off any turn.
  //
  // `userId` is unwritten at all three today: none of them passes it, though
  // the two routes hold a real `session.user.id` behind `withAuth`.
  'lib/framework/resparkable/services/ideate.ts | agentId=spread((agent?.id ? { agentId: agent.id } : {})) | conversationId=— | workflowExecutionId=— | userId=—',
