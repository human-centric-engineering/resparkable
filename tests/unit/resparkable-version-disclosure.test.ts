 ...
 * platform version.
 *
 * `RESPARKABLE_VERSION` names the exact upstream release a deployment runs, and
 ...
 * `components/`; a chain through `hooks/`; `await import('@/lib/resparkable-version')`;
...
const IMPORTS_VERSION = /['"]@\/lib\/resparkable-version['"]/;

/**
 * `@/`-aliased specifiers, static and dynamic. `import(` is included because
 * `await import('@/lib/resparkable-version')` was neither seeded nor followed by
...
/**
 * Every module whose import graph reaches `@/lib/resparkable-version`, as a reverse
...
describe('RESPARKABLE_VERSION disclosure', () => {
  ...
  it('lets no client component reach it, so it never enters the public bundle', () => {
    // `lib/resparkable-version.ts` is deliberately NOT `server-only` — the MCP tier
