 ...
 *
 * The fixture deliberately gives `appVersion` and `resparkableVersion` **different**
...
const RESPARKABLE_VERSION_FIXTURE = '0.9.0';
...
function statsFixture(overrides: Partial<SystemStats['system']> = {}): SystemStats {
  return {
    ...
    system: {
      ...
      resparkableVersion: RESPARKABLE_VERSION_FIXTURE,
...
describe('SystemInfo', () => {
  it('renders the Resparkable platform version', () => {
    render(<SystemInfo stats={statsFixture()} />);

    expect(screen.getByText('Resparkable platform')).toBeInTheDocument();
    expect(screen.getByText(RESPARKABLE_VERSION_FIXTURE)).toBeInTheDocument();
  });
  ...
  it('renders the app version separately from the platform version', () => {
    ...
    expect(screen.getByText(RESPARKABLE_VERSION_FIXTURE)).toBeInTheDocument();
    expect(APP_VERSION_FIXTURE).not.toBe(RESPARKABLE_VERSION_FIXTURE);
  });
  ...
  it('keeps the two labels distinct when both rows carry the same version', () => {
    // Upstream, `BRAND.name` IS the platform name and `APP_VERSION` equals
    // `RESPARKABLE_VERSION` — so an un-disambiguated card renders the same label
    ...
    // ships from.
    //
    // `BRAND.name` is pinned to the unconfigured default by tests/setup.ts, so
    // the app row reads "<default> app" and the bare name must not appear alone.
    const collidingBrand = BRAND.name;
    render(<SystemInfo stats={statsFixture({ appVersion: RESPARKABLE_VERSION_FIXTURE })} />);
    ...
    expect(screen.getByText('Resparkable platform')).toBeInTheDocument();
    expect(screen.getAllByText(RESPARKABLE_VERSION_FIXTURE)).toHaveLength(2);
  });
  ...
  it('renders "unknown" for a version the payload omits, not an empty line', () => {
    // `SystemStats` says `resparkableVersion` is required, but nothing enforces that
    ...
    const withoutPlatformVersion = JSON.parse(
      JSON.stringify(statsFixture(), (key, value) =>
        key === 'resparkableVersion' ? undefined : value
      )
    ) as SystemStats;
    expect(withoutPlatformVersion.system).not.toHaveProperty('resparkableVersion');
  ...
  it('says so when stats could not be fetched, rather than rendering an empty card', () => {
    ...
    expect(screen.getByText(/System information is unavailable/i)).toBeInTheDocument();
    expect(screen.queryByText('Resparkable platform')).not.toBeInTheDocument();
    expect(screen.queryByText(RESPARKABLE_VERSION_FIXTURE)).not.toBeInTheDocument();
  });
});
