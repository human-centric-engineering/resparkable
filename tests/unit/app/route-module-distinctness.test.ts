...
export const ALLOWED_IDENTICAL_GROUPS: readonly { paths: readonly string[]; reason: string }[] = [
  // FORK (Resparkable): the case the docblock above predicts, arrived. These
  // are one-line `loading.tsx` skeletons that already delegate to the shared
  // `<SkeletonList>` — the implementation IS shared, by import, which is the
  // spelling this guard asks for. What is left to be identical is the four
  // lines it takes to call it, and there is no further factoring available:
  // Next resolves `loading.tsx` by position in the tree, so each route segment
  // needs its own file whatever that file contains.
  //
  // Declared rather than perturbed. Adding a distinguishing comment to each
  // would green the run and cost the guard its point, since a comment added to
  // silence it is indistinguishable from the clobber it exists to catch.
  {
    paths: [
      'app/(resparkable)/resparkable/areas/loading.tsx',
      'app/(resparkable)/resparkable/entities/[id]/loading.tsx',
      'app/(resparkable)/resparkable/entities/loading.tsx',
      'app/(resparkable)/resparkable/goals/loading.tsx',
      'app/(resparkable)/resparkable/projects/[id]/loading.tsx',
      'app/(resparkable)/resparkable/projects/loading.tsx',
    ],
    reason:
      'Six list routes whose loading state is the same five-row skeleton, rendered through the shared <SkeletonList rows={5} label="Loading" />. Next needs a loading.tsx per segment, so the call site cannot be deduplicated further.',
  },
  {
    paths: [
      'app/(resparkable)/resparkable/boards/[slug]/loading.tsx',
      'app/(resparkable)/resparkable/boards/loading.tsx',
    ],
    reason:
      'The board index and a single board share one loading skeleton, through the same shared component. Same reason as the group above.',
  },
  {
    paths: [
      'app/(resparkable)/resparkable/connections/loading.tsx',
      'app/(resparkable)/resparkable/documents/loading.tsx',
      'app/(resparkable)/resparkable/graph/loading.tsx',
      'app/(resparkable)/resparkable/sharing/loading.tsx',
    ],
    reason:
      'Four routes whose loading state is the same four-row skeleton, through the same shared component. Same reason as the two groups above; the row count is what separates this group from the first.',
  },
];
