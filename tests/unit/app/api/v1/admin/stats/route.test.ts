...
// and `RESPARKABLE_VERSION` both track the release — so a test asserting only
// `resparkableVersion === RESPARKABLE_VERSION` passes just as happily against a route
...
describe('GET /api/v1/admin/stats', () => {
  ...
  describe('System Information', () => {
    ...
    it('should include the Sunrise platform version, equal to RESPARKABLE_VERSION', async () => {
      ...
      // `tests/unit/resparkable-version-disclosure.test.ts`, not in a count.
      //
      // Asserted against the imported constant, with `APP_VERSION` mocked to a
      // different number above so "reads RESPARKABLE_VERSION" and "echoes whatever
      // the app version is" are distinguishable outcomes.
      const response = await GET(dummyRequest);
      const data = await parseResponse<SuccessResponse>(response);

      expect(response.status).toBe(200);
      expect(data.data.system.resparkableVersion).toBe(RESPARKABLE_VERSION);
      expect(data.data.system.resparkableVersion).not.toBe(data.data.system.appVersion);
    });
