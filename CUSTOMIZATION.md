...
**This fork already patches the version-file read.** The guard reads its
claim from `lib/sunrise-version.ts` / `SUNRISE_VERSION` by default, which
Resparkable renamed at the Obsiddy→Resparkable rebrand. `fork-sync-integrity.yml`
sets `VERSION_FILE=lib/resparkable-version.ts` and
`VERSION_CONST=RESPARKABLE_VERSION` so the check actually runs here instead
of always skipping — see [`sunrise-asks.md`](./.context/framework/resparkable/sunrise-asks.md)
ask #40 if you rename the file again.
